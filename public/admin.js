...
async function renderPosts() {
  ...
  body.innerHTML = `
    <div style="display:flex;gap:8px;margin-bottom:16px">
      <button class="btn small ${postsFilter==="pending"?"":"secondary"}" data-pf="pending">待审核 (${pendingCount})</button>
      <button class="btn small ${postsFilter==="approved"?"":"secondary"}" data-pf="approved">已通过</button>
      <button class="btn small ${postsFilter==="all"?"":"secondary"}" data-pf="all">全部 (${posts.length})</button>
    </div>
    ${filtered.length === 0 ? '<p style="color:var(--muted);text-align:center;padding:40px 0">暂无留言</p>' : filtered.map(p => `
      <div style="background:var(--card,#fff);border:1px solid var(--line);border-radius:12px;padding:16px 18px;margin-bottom:12px">
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
          <strong style="font-size:14px">${esc(p.author)}</strong>
          ...
          <span style="font-size:12px;color:var(--muted)">· ${esc((p.createdAt||"").slice(0,10))}</span>
          ${(p.status||"approved")!=="approved" ? '<span style="font-size:12px;color:#c8742c;background:#fdf3ea;padding:2px 8px;border-radius:10px">待审核</span>' : '<span style="font-size:12px;color:#2e7d32;background:#edf7ed;padding:2px 8px;border-radius:10px">已通过</span>'}
        </div>
        <div style="font-size:14px;line-height:1.7;margin-top:8px;white-space:pre-wrap">${esc(p.content)}</div>
        ${(p.images||[]).length ? '<div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">' + p.images.map(u => `<img src="${esc(u)}" style="width:100px;height:75px;object-fit:cover;border-radius:6px;cursor:pointer" onclick="window.open('${esc(u)}')">`).join("") + '</div>' : ''}
