...
const DATA_DIR = process.env.DATA_DIR || (STORAGE_ROOT ? path.join(STORAGE_ROOT, "data") : path.join(ROOT, "data"));
...
function safeUploadPath(urlPath) {
  const decoded = decodeURIComponent(urlPath.split("?")[0]);
  const clean = decoded.replace(/^\/uploads\/?/, "").replace(/^\/+/, "");
  const full = path.normalize(path.join(UPLOAD_DIR, clean));
...
function handleStatic(req, res, pathname) {
  if (pathname === "/healthz") {
    sendJson(res, 200, {
      ok: true,
      service: "silver-community-h5",
      storage: {
        dataDir: DATA_DIR,
        uploadDir: UPLOAD_DIR,
        volume: Boolean(STORAGE_ROOT)
      }
    });
    return;
  }
  ...
  if (pathname.startsWith("/uploads/")) {
    const filePath = safeUploadPath(pathname);
    if (!filePath) {
