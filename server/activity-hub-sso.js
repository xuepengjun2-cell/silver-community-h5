...
function verifyActivityHubSsoToken(token, { secret, now = Math.floor(Date.now() / 1000) } = {}) {
  ...
  if (claims.v !== 1 || claims.iss !== "kkhc-channel-assistant-v2" || claims.aud !== "silver-activity") throw ssoError("免密入口票据来源不正确");
  ...
  if (!/^[A-Za-z0-9_.:-]{1,128}$/.test(subject) || !/^[a-z0-9_.-]{3,40}$/.test(username)
    || !/^[a-f0-9]{32,128}$/i.test(jti) || !Number.isInteger(issuedAt)
    || !Number.isInteger(expiresAt) || !Number.isInteger(accessExpiresAt)) throw ssoError("免密入口票据身份信息不完整");
  if (issuedAt > now + MAX_CLOCK_SKEW_SECONDS || expiresAt <= now || expiresAt <= issuedAt
    || expiresAt - issuedAt > MAX_TICKET_SECONDS || accessExpiresAt < expiresAt || accessExpiresAt <= now) throw ssoError("免密入口票据已过期或无效");
  return { subject, username, displayName: String(claims.n || username).slice(0, 60), issuedAt, expiresAt, accessExpiresAt, jti };
}
...
function resolveActivityHubUser(db, claims, userMap = {}) {
  ...
  return users.find(user => user && user.status === "active"
...
function resolveOrProvisionActivityHubUser(db, claims, userMap = {}) {
  ...
  const user = {
    id, username: claims.username, name: claims.displayName || claims.username,
    role: "member", status: "active", canDownload: true,
    ssoSubject: claims.subject, ssoUsername: claims.username,
    authSource: "kkhc-channel-assistant-v2", createdAt: new Date().toISOString()
  };
  users.push(user);
