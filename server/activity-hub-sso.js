...
function verifyActivityHubSsoToken(token, { secret, now = Math.floor(Date.now() / 1000) } = {}) {
  ...
  if (!/^[A-Za-z0-9_.:-]{1,128}$/.test(subject)
    ...
    || !Number.isInteger(issuedAt) || !Number.isInteger(expiresAt) || !Number.isInteger(accessExpiresAt)) {
    throw ssoError("免密入口票据身份信息不完整");
  }
  if (issuedAt > now + MAX_CLOCK_SKEW_SECONDS || expiresAt <= now || expiresAt <= issuedAt
    || expiresAt - issuedAt > MAX_TICKET_SECONDS || accessExpiresAt < expiresAt || accessExpiresAt <= now) {
...
function resolveActivityHubUser(db, claims, userMap = {}) {
  ...
  const usernames = new Set([claims.username, String(mappedUsername || "").trim().toLowerCase()].filter(Boolean));
  return users.find(user => user && user.status === "active"
    && (String(user.id) === claims.subject || String(user.ssoSubject || "") === claims.subject
...
function resolveOrProvisionActivityHubUser(db, claims, userMap = {}) {
  ...
  const identityCollision = users.find(user => user && (
